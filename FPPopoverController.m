...
-(void)presentPopoverFromPoint:(CGPoint)fromPoint
{
    ...
    [self setupView];
    self.view.alpha = 1.0;
    _contentView.layer.anchorPoint = CGPointMake(.5, 0);
    
    CATransform3D translate = CATransform3DIdentity;
    translate.m34 = -1/500;
    translate = CATransform3DTranslate(translate, 0, -100, 150);
    
    CATransform3D totrans = CATransform3DIdentity;
    totrans.m34 = -1/500;
    totrans = CATransform3DTranslate(totrans, 0, -150, 0);
    
    CATransform3D transform = CATransform3DIdentity;
    transform.m34 = -1.0f/500.0f;
    transform = CATransform3DRotate(transform, 72 * M_PI/180, 1.0f, 0.0f, 0.0f);
    
    CATransform3D toRotate = CATransform3DIdentity;
    toRotate.m34 = -1/500;
    toRotate = CATransform3DRotate(toRotate, 0*M_PI/180 , 1, 0, 0);
    
    CATransform3D both = CATransform3DConcat(translate, transform);
    _contentView.layer.transform = both;
    
    CATransform3D toBoth = CATransform3DConcat(totrans, toRotate);
    
    CATransform3D undo = CATransform3DIdentity;
    undo.m34 = -1.0f/500.0f;
    undo = CATransform3DRotate(undo, 0, 1.0f, 0.0f, 0.0f);
    
    CABasicAnimation *bothAnim = [CABasicAnimation animationWithKeyPath:@"transform"];
    bothAnim.toValue = [NSValue valueWithCATransform3D:totrans];
    bothAnim.duration = 1;
    bothAnim.fillMode = kCAFillModeForwards;
    bothAnim.removedOnCompletion = NO;
    //[_contentView.layer addAnimation:bothAnim forKey:nil];
    
    self.view.alpha = 0;
    
    [UIView animateWithDuration:0.2 animations:^{
        _contentView.layer.transform = toBoth;
        self.view.alpha = 1.0;
    }];
